...
metabolite_data = pd.read_csv("facs_plot.csv")  # Replace 'your_file.csv' with the actual file path
...
for i, (metabolite, ax) in enumerate(zip(unique_metabolites, axes), 1):
    ...
    ax.set_ylabel('Relative aggregation [%]' if i == 1 else '')
    if metabolite == 'Threonine':
...
plt.tight_layout()
plt.savefig('facs.svg', format='svg')
plt.show()
